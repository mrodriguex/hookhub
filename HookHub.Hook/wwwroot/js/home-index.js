...
function renderHookInfo(data) {
    ...
    tbody.innerHTML = `
        <tr>
            ...
            <td>${data.hookConnection.hookName}</td>
        </tr>
        <tr>
        ...
        <tr>
            <td><strong>Last Keep Alive</strong></td>
            <td>${new Date(data.hookConnection.lastKeepAlive).toISOString().replace(/\.\d{3}Z$/, 'Z')}</td>
        </tr>
        <tr>
            <td><strong>Keep Alive Interval (ms)</strong></td>
            <td>${data.hookConnection.timeIntervals_KeepAlive}</td>
        </tr>
    `;
}
