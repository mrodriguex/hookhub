...
function tokens(componentID, vars, vals) {
    variables = String(vars).split('|');
    valores = String(vals).split('|');
    var element = document.getElementsByClassName('atwho-container');
...
function validaXpression(ecuacion) {
    var ejecutar;

    if (ecuacion.trim() == '$') { return false; }

    try {
        ecuacion = ecuacion.replace(new RegExp('\{([^\}]+)}', 'g'), '');
        ecuacion = ecuacion.replace(new RegExp('\\' + ejecutor, 'g'), '');
        ecuacion = ecuacion.replace(/\n/g, '');
        ecuacion = ecuacion.replace(/(\r\n|\n|\r)/gm, "");
        ecuacion = ecuacion.replace(/FechaCaptura/g, '0');
        for (var i = 0; i < variables.length; i++) {
            ecuacion = ecuacion.replace(new RegExp(escapar(variables[variables.length - i - 1].trim()), 'g'), '(' + valores[variables.length - i - 1] + ')');
        }
    } catch (err) {
    }
    ...
    if (ecuacion == "") { return false; }

    try {
        ejecutar = eval(ecuacion + ';');
        return true;
    } catch (err) {
        return false;
    }
